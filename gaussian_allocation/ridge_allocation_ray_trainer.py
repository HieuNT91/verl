...
class RidgeAllocation:
    def __init__(self, 
        ...
        """
        Ridge regression-based allocation model.
        
        Args:
            embedding_matrix: (n_questions, embedding_dim) matrix of question embeddings
            qid_to_idx: dict mapping question ID to index in embedding matrix
            idx_to_qid: dict mapping index to question ID
    ...
    def fit(self, train_qids, observations):
        """
        Fit ridge regression model.
        
        Args:
            train_qids: List of question IDs for training
            observations: Array of target values (accuracies)
        """
        if self.qid_to_idx is None:
            raise ValueError("qid_to_idx mapping is not provided.")
        
        # Get embeddings for training questions
        train_indices = [self.qid_to_idx[str(qid)] for qid in train_qids if str(qid) in self.qid_to_idx]
        
        if len(train_indices) == 0:
            raise ValueError("No valid training questions found in embedding matrix")
        
        X_train = self.embedding_matrix[train_indices]
        y_train = np.array(observations)
    ...
    def predict(self, test_qids):
        """
        Predict accuracies for test questions.
        
        Args:
            test_qids: List of question IDs for prediction
            
        Returns:
            numpy array of predicted accuracies
        """
        ...
        # Get embeddings for test questions
        test_indices = [self.qid_to_idx[str(qid)] for qid in test_qids if str(qid) in self.qid_to_idx]
        
        if len(test_indices) == 0:
            return np.array([])
        
        X_test = self.embedding_matrix[test_indices]
        ...
        # Predict
        predictions = self.ridge_model.predict(X_test)
...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    def _ensure_ridge_artifacts(self):
        """Ensure ridge regression artifacts are loaded"""
        if self._gpr_ready:  # Reuse the same flag for consistency
            return
        
        # Load embedding matrix instead of pairwise distances
        embedding_path = os.path.join(self.gpr_data_root, f"embeddings_{self.gpr_embedder.replace('/','-')}_{self.gpr_dataset}.npy")
        indices_path = os.path.join(self.gpr_data_root, f"indices_{self.gpr_embedder.replace('/','-')}_{self.gpr_dataset}.json")
        
        if not (os.path.exists(embedding_path) and os.path.exists(indices_path)):
            print(f"Ridge artifacts not found: {embedding_path} or {indices_path}")
            self._gpr_ready = False
            return
            
        import json as _json
        self._gpr_pairwise = np.load(embedding_path)  # Now contains embeddings, not pairwise distances
    ...
    def _ridge_allocate_repeat_times(self, batch_dict):
        """Use Ridge regression for allocation instead of GPR"""
        question_uuids = batch_dict['index']
        if not isinstance(question_uuids, (list, tuple, np.ndarray)):
            question_uuids = [question_uuids]
        ...
        if not self._last_batch_accs:
            print("[Ridge] Skipping: no last-batch accuracies available yet.")
            return None
            
        self._ensure_ridge_artifacts()
        if not self._gpr_ready:
            print("[Ridge] Skipping: Ridge artifacts not ready (embeddings/indices missing).")
            return None
        ...
        # Current questions to predict for
        ...
        ridge_alpha = getattr(self.config.algorithm, 'ridge_alpha', 1.0)
        ridge_model = RidgeAllocation(
            embedding_matrix=self._gpr_pairwise,  # Now contains embeddings
            qid_to_idx=self._gpr_qid_to_idx,
            idx_to_qid={i: q for q, i in self._gpr_qid_to_idx.items()},
            alpha=ridge_alpha,
            normalize_features=True
        )
        
        try:
            ridge_model.fit(train_qids, y_train)
            mean_pred = ridge_model.predict(kept_current)
        except Exception as e:
            print(f"[Ridge] Error during fitting/prediction: {e}")
            return None
        ...
        print(f"[Ridge] Predictions: {mean_pred[:5]}...")  # Show first 5 predictions
        print(f"[Ridge] Allocations: {allocated[:5]}...")  # Show first 5 allocations

        # Map back to full question_uuids list
        repeat_times: dict = {}
    ...
    def _calculate_adaptive_repeat_times(self, batch_dict):
        """
        Calculate adaptive repeat times - now using Ridge regression instead of GPR.
        """
        question_uuids = batch_dict['index']
        if not isinstance(question_uuids, (list, tuple, np.ndarray)):
            question_uuids = [question_uuids]
        
        # If Ridge-based allocation is enabled and artifacts available, try it first
        ridge_alloc = self._ridge_allocate_repeat_times(batch_dict)
        if ridge_alloc is not None:
            return ridge_alloc 
        else:
